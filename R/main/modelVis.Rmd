...
```{r load-packages}
library(knitr)
source("~/nhsx/stmnhsx/R/main/libraries.R", local =  knitr::knit_global())
# source("~/nhsx/stmnhsx/R/main/modelSelection.R", local = knitr::knit_global()) 
```

```{r, initalise-wordnet-dictionary}
library(wordnet)
initDict()
...
```{r}
stm::toLDAvis(mod = model25, docs = stmdata$documents, open.browser = interactive())
```

This code displays the topic labels for the topics usign sageLabels() function. This displays the real words (as opposed to the processed words) most associated with each topic. findThoughts() then displays the most representative texts for each topic. 
```{r, Topic labels}
sageLabels(model25)
...
Using `findTopics` topics can be searched for a list of search terms specified by the user. This returns the number of topics and the topic that contains words in the list. 
...
findlist <- strsplit(user.input, ",")
#print(findlist[[1]])
findTopic(model25, findlist[[1]]) # check function
```
